...
corridor_tracts <- tigris::tracts(
 ...
 select(GEOID)
library(rgdal)
corridor_geo <- readOGR('data-raw/Baltimore_corridor_crosswalk',
                     verbose=FALSE)
corridor_tracts<-corridor_tracts%>%
  merge(corridor_tracts, corridor_geo, by.x=tract_string, by.y=GEOID)
  select(GEOID,Corridor_Name, geometry)%>%
...
if(metro == "pdx") {
  ...
  eva_data_main <- read_xlsx(paste0("./data-raw/", metro, ".xlsx"),
    ...
    gather("variable", "raw_value", -tract_string, -Corridor_Name) %>%
    group_by(variable) %>%
