...
## corridor tract sets ---------------
corridor_tracts <- readOGR('data-raw/Corridortractv1', 'Export_Output',
                     verbose=FALSE
)
corridor_tracts<-st_as_sf(corridor_tracts)
corridor_tracts<-corridor_tracts%>%
  select(GEOID, OKR_Area_N)%>%
  group_by(OKR_Area_N)%>%
  summarise(geometry=st_union(geometry))%>%
  rename(corridor_name=OKR_Area_N)%>%
